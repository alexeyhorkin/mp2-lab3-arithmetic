...
bool determinateoperand(char s) //если char это  *+/-, то вернЄм true;
{
	int k = s;
...
bool determinateopetator(char s) //если char это () *+/-, то вернЄм true;
{
	int k = s;
...
//						flag2 = 1;
//					}
//					// проверить флаг и помен€ть знак, если надо
...
int ChekString(const string a)
{
	...
	for (int i = 0; i < a.length(); i++) //два повтор€ющихс€ операторанда 
	{
